...
function Start-AsAdministrator {
    if (-not (Test-Administrator)) {
        ...
        $arguments = "-NoExit -ExecutionPolicy Bypass -File `"$($script:ScriptPath)`""
        ...
        Write-Log "Starting elevated process with arguments: $arguments"
...
function Start-SetupMode {
    ...
    if (Set-BackupConfig -Config $config) {
        ...
        # Setup Task Scheduler (Automatic - no question)
        Write-Host "`nSetting up automatic scheduling..." -ForegroundColor Yellow
        if (New-ScheduledTask) {
            Write-Host "Scheduled task created successfully!" -ForegroundColor Green
            Write-Host "Service will run automatically at system startup and user logon." -ForegroundColor Gray
...
function New-ScheduledTask {
    Write-Log "New-ScheduledTask called. Admin check..."
    
    if (-not (Test-Administrator)) {
        Write-Log "Not running as admin - cannot create task" "ERROR"
        return $false
    }
    
    Write-Log "Verified: Running as admin"
    ...
    Write-Log "Task does not exist. Creating new task..."
    
    try {
        # Create task action - use quoted path to handle spaces in path
        Write-Log "Creating task action with script path: $script:ScriptPath"
        ...
        Write-Log "Creating task triggers..."
        $trigger1 = New-ScheduledTaskTrigger -AtStartup
        $trigger2 = New-ScheduledTaskTrigger -AtLogOn
        $triggers = @($trigger1, $trigger2)
        
        # Create task principal - use SYSTEM for elevated privileges
        Write-Log "Creating task principal with SYSTEM account..."
        $principal = New-ScheduledTaskPrincipal -UserId "SYSTEM" -LogonType ServiceAccount -RunLevel Highest
        
        # Create task settings
        Write-Log "Creating task settings..."
        $settings = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -StartWhenAvailable -DontStopOnIdleEnd
        
        # Register the task
        Write-Log "Registering scheduled task: $script:TaskName"
        $registerResult = Register-ScheduledTask -TaskName $script:TaskName -Action $action -Trigger $triggers -Principal $principal -Settings $settings -Description "Advanced Folder Backup Tool - Real-time monitoring and archiving" -Force -ErrorAction Stop
        ...
        Write-Host "   - System Startup" -ForegroundColor Gray
        Write-Host "   - User Logon" -ForegroundColor Gray
        
        # Verify it was created
        Start-Sleep -Milliseconds 500
        $verify = Get-ScheduledTask -TaskName $script:TaskName -ErrorAction SilentlyContinue
        if ($verify) {
            Write-Log "Task verification: SUCCESS"
            return $true
        }
        else {
            Write-Log "Task creation reported success but verification failed!" "WARN"
            return $true  # Return true anyway since registration succeeded
        }
    }
    catch {
        Write-Log "CRITICAL: Error creating scheduled task: $($_.Exception.Message)" "ERROR"
        Write-Log "Full error: $($_ | Out-String)" "ERROR"
        Write-Host "❌ Error creating task: $($_.Exception.Message)" -ForegroundColor Red
        Write-Host "Note: If on domain-joined computer, may need domain admin rights" -ForegroundColor Yellow
        return $false
    }
}
...
function Start-BackupService {
    ...
    try {
        ...
        if ($task) {
            # Task exists - enable and start it
            ...
            Write-Log "Scheduled task not found. Setting up..."
            ...
            if (Test-Administrator) {
                ...
                if (New-ScheduledTask) {
                    ...
                    # Run task immediately
                    Start-ScheduledTask -TaskName $script:TaskName -ErrorAction SilentlyContinue
                    Write-Log "Task created and started immediately"
                }
            }
            else {
                # Request admin privileges to create task
                Write-Host "Admin privileges required to create scheduled task." -ForegroundColor Yellow
                Write-Host "Please run this script as Administrator or use StartBackup.bat." -ForegroundColor Yellow
                Write-Log "Attempted to create task without admin privileges"
                return $false
            }
        }
    ...
    try {
        ...
        try {
            while ($true) {
                ...
                # Periodic archive management check (once per day)
                if ($script:LastArchiveTime) {
                    $timeSinceLastArchive = (Get-Date) - $script:LastArchiveTime
                    if ($timeSinceLastArchive.TotalHours -ge 24) {
                        Write-Log "Running daily archive management..."
                        Invoke-ArchiveManagement -Config $Config
                        $script:LastArchiveTime = Get-Date
                    }
                }
...
function Start-BackgroundMode {
    ...
    # Track time for periodic archive management
    $script:LastArchiveTime = Get-Date
...
function Update-BackupSourcePath {
    ...
    if (Set-BackupConfig -Config $Config) {
        ...
        Write-Host "`nStarting backup service in background..." -ForegroundColor Yellow
        Start-BackupService -Config $Config
        Write-Host "Backup service is now RUNNING" -ForegroundColor Green
        return $Config
    }
    return $null
}
...
function Show-SettingsMenu {
    ...
    while ($true) {
        ...
        switch ($choice) {
            "1" { $newConfig = Update-BackupSourcePath -Config $Config; if ($newConfig) { $Config = $newConfig; Read-Host "`nPress Enter to continue" } }
            "2" { $newConfig = Update-BackupDestinationPath -Config $Config; if ($newConfig) { $Config = $newConfig; Read-Host "`nPress Enter to continue" } }
            "3" { $newConfig = Update-RetentionDays -Config $Config; if ($newConfig) { $Config = $newConfig; Read-Host "`nPress Enter to continue" } }
    ...
    if ($Control) {
        ...
        switch ($Control) {
            ...
            "Status" {
                $task = Get-ScheduledTask -TaskName $script:TaskName -ErrorAction SilentlyContinue
                if ($task) {
                    # If task is disabled or stopped, try to enable and start it (requires admin)
                    if (-not $task.Enabled) {
                        Write-Log "Task found but disabled - attempting to enable..."
                        try {
                            Enable-ScheduledTask -TaskName $script:TaskName -ErrorAction SilentlyContinue | Out-Null
                            $task = Get-ScheduledTask -TaskName $script:TaskName -ErrorAction SilentlyContinue
                        }
                        catch {
                            Write-Log "Could not enable task: $($_.Exception.Message)"
                        }
                    }
                    
                    # Start the task if not already running
                    if ($task.State -ne "Running" -and $task.State -ne "Running") {
                        Write-Log "Task not running - attempting to start..."
                        try {
                            Start-ScheduledTask -TaskName $script:TaskName -ErrorAction SilentlyContinue | Out-Null
                            Start-Sleep -Milliseconds 500
                            $task = Get-ScheduledTask -TaskName $script:TaskName -ErrorAction SilentlyContinue
                        }
                        catch {
                            Write-Log "Could not start task: $($_.Exception.Message)"
                        }
                    }
                    
                    $statusText = if ($task.Enabled -and ($task.State -eq "Running" -or $task.State -eq "Ready")) { "RUNNING" } else { "STOPPED" }
                    Write-Host "Service Status: $statusText" -ForegroundColor $(if ($task.Enabled -and ($task.State -eq "Running" -or $task.State -eq "Ready")) { "Green" } else { "Yellow" })
                    Write-Log "Task exists: $statusText"
                }
                else {
                    # Task doesn't exist - create it automatically
                    if ($config) {
                        Write-Host "Service Status: STOPPED (creating task...)" -ForegroundColor Yellow
                        Write-Log "No task found. Attempting creation..."
                        
                        if (-not (Test-Administrator)) {
                            Write-Log "Admin privileges required. Requesting elevation..."
                            Write-Host "Admin privileges required. Restarting..." -ForegroundColor Yellow
                            Start-AsAdministrator
                            exit 0
                        }
                        
                        # We have admin - create the task
                        Write-Host "Creating scheduled task with admin rights..." -ForegroundColor Cyan
                        $taskCreated = New-ScheduledTask
                        
                        if ($taskCreated -eq $true) {
                            # Task creation returned success, now verify it exists
                            Start-Sleep -Seconds 1
                            $verifyTask = Get-ScheduledTask -TaskName $script:TaskName -ErrorAction SilentlyContinue
                            
                            if ($verifyTask) {
                                Write-Host "Service Status: RUNNING ✅" -ForegroundColor Green
                                Write-Log "Task created successfully and verified"
                                
                                # Enable it if it's not enabled
                                if (-not $verifyTask.Enabled) {
                                    Enable-ScheduledTask -TaskName $script:TaskName -ErrorAction SilentlyContinue | Out-Null
                                    Write-Log "Task was disabled, enabling now..."
                                }
                            }
                            else {
                                Write-Host "❌ Task creation reported success but verification failed!" -ForegroundColor Red
                                Write-Log "Task creation verification FAILED"
                            }
                        }
                        else {
                            Write-Host "❌ Failed to create scheduled task" -ForegroundColor Red
                            Write-Log "Task creation returned: $taskCreated"
                        }
                    }
                    else {
                        Write-Host "Service not configured" -ForegroundColor Yellow
                        Write-Host "Please run BackupControl.bat first." -ForegroundColor Yellow
                        Write-Log "No config found for status check"
                    }
                }
                exit 0
            }
    ...
    elseif ($Mode -eq "Background" -or ($config -and (Test-ScheduledTask))) {
        ...
        elseif ($config -and (Test-ScheduledTask)) {
            ...
            if ($Mode -eq "Auto") {
                ...
                if ($config) {
                    # Config exists but no scheduled task - create it automatically
                    Write-Log "Configuration exists but no scheduled task. Creating task..."
                    Write-Host "Configuration found. Creating scheduled task..." -ForegroundColor Yellow
                    
                    if (Test-Administrator) {
                        if (New-ScheduledTask) {
                            Write-Host "✅ Scheduled task created successfully!" -ForegroundColor Green
                            Write-Host "Service will start automatically at:" -ForegroundColor Green
                            Write-Host "   - System startup" -ForegroundColor Gray
                            Write-Host "   - User logon" -ForegroundColor Gray
                        ...
                        Write-Host "⚠️ Admin privileges required to create scheduled task." -ForegroundColor Yellow
                        Write-Host "Please run as Administrator (use StartBackup.bat)" -ForegroundColor Yellow
                        Show-MainMenu -Config $config
                    }
                }
                else {
